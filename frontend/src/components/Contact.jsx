...
// Dynamically load all 80 WebP frames from the signature directory
const imagesContext = import.meta.glob('../assets/signature/*.webp', { eager: true });
const frameUrls = Object.keys(imagesContext).sort().map(key => imagesContext[key].default || imagesContext[key]);
...
const SocialIcon = ({ type }) => {
    ...
    switch (type) {
        case 'whatsapp':
            return (
                <svg className="w-5 h-5 md:w-7 md:h-7 shrink-0" viewBox="0 0 24 24" fill="currentColor">
...
const MobileIconBox = ({ type, hoverColor = "hover:text-[#ff0000] active:text-[#ff0000]" }) => (
    <a href={links[type]} target="_blank" rel="noreferrer"
        className={`bg-white text-black border-[3px] border-black p-2 md:p-3 shadow-[4px_4px_0_rgba(0,0,0,1)] hover:shadow-[6px_6px_0_rgba(0,0,0,1)] hover:-translate-y-1 hover:rotate-3 transition-all duration-300 flex items-center justify-center cursor-pointer pointer-events-auto z-50 ${hoverColor}`}>
        <SocialIcon type={type} />
    </a>
);
...
const Contact = () => {
    ...
    const imagesRef = useRef([]); // Holds fully decoded Image objects in RAM
    const [imagesLoaded, setImagesLoaded] = useState(false);

    // Preload all 80 frames into browser RAM instantly on mount
    useEffect(() => {
        let loadedCount = 0;
        ...
        frameUrls.forEach((url, index) => {
            ...
            img.onload = () => {
                ...
                if (loadedCount === frameUrls.length) {
                    ...
                    // Immediately paint the very first frame to the canvas when ready
                    if (canvasRef.current) {
                        const ctx = canvasRef.current.getContext('2d');
                        // Set internal canvas resolution to match the image dimensions
                        canvasRef.current.width = preloadedImages[0].width;
                        canvasRef.current.height = preloadedImages[0].height;
                        ctx.drawImage(preloadedImages[0], 0, 0);
                    }
                }
            };
    ...
    // Get scroll progress relative to this specific section
    const { scrollYProgress } = useScroll({
        target: containerRef,
        offset: ["start center", "end end"] // Starts when top of section hits middle of screen, ends at the absolute bottom
    });

    // Apply a softer spring physics engine to eliminate scroll jumpiness
    const smoothProgress = useSpring(scrollYProgress, {
        stiffness: 60,  // Faster reaction time to completely eliminate the "delay/lag" feeling
        damping: 20,    // High damping keeps it completely smooth without jumping
        restDelta: 0.001
    });

    // Scrub the canvas using the buttery-smooth spring progress
    useMotionValueEvent(smoothProgress, "change", (latest) => {
        if (!imagesLoaded || !canvasRef.current || imagesRef.current.length === 0) return;

        // Map scroll 0-1 to frame 0-79
        const frameIndex = Math.min(
            frameUrls.length - 1,
            Math.max(0, Math.round(latest * (frameUrls.length - 1)))
        );
    ...
    // Mobile specific: state for layout tweaks
    ...
    return (
        <div ref={containerRef} className="pt-[5vh] md:pt-[25vh]">
            <section id="contact" className="relative w-full h-[100dvh] flex flex-col items-center justify-center overflow-hidden">
                ...
                <div className="absolute inset-0 overflow-hidden z-0 flex items-center justify-center">
                    ...
                    <div className="absolute w-[200vw] md:w-[150vw] bg-[#facc15] border-y-[3px] md:border-y-[4px] border-black py-2 md:py-4 transform rotate-[8deg] md:rotate-[10deg] shadow-[0_6px_0_rgba(0,0,0,1)] md:shadow-[0_10px_0_rgba(0,0,0,1)] flex overflow-hidden pointer-events-auto">
                        <motion.div style={{ x: yellowX }} className="flex w-full">
                            <div className="flex items-center whitespace-nowrap w-max">
                                ...
                                {[...Array(40)].map((_, i) => (
                                    <div key={`yellow-${i}`} className="flex items-center gap-4 md:gap-20 px-4 md:px-10 shrink-0 text-black">
                                        <span className="font-sans font-black text-xl sm:text-2xl md:text-3xl lg:text-4xl uppercase tracking-wider">
                    ...
                    <div className="absolute w-[200vw] md:w-[150vw] bg-[#ff0000] border-y-[3px] md:border-y-[4px] border-black py-2 md:py-4 transform rotate-[-15deg] md:rotate-[-10deg] shadow-[0_6px_0_rgba(0,0,0,1)] md:shadow-[0_10px_0_rgba(0,0,0,1)] flex overflow-hidden">
                        <motion.div style={{ x: redX }} className="flex w-full">
                            <div className="flex items-center whitespace-nowrap w-max">
                                {[...Array(40)].map((_, i) => (
                                    <div key={`red-${i}`} className="flex items-center gap-4 md:gap-20 px-4 md:px-10 shrink-0 text-white">
                                        <span className="font-sans font-black text-xl sm:text-2xl md:text-3xl lg:text-4xl uppercase tracking-wider">
                                            "With great power comes great responsibilities..!"
                                        </span>
                                    </div>
                ...
                <div className="relative z-10 max-w-6xl w-full mx-auto flex flex-col justify-center items-center px-6 md:px-12 h-full py-4 md:py-8 pointer-events-none">
                    ...
                    {/* Signature Video & CTA */}
                    <div className="w-full relative flex flex-col items-center justify-center mt-2 mb-2 z-20 pointer-events-none">

                        {/* Signature Video Comic Panel */}
                        <div className="relative w-[45vw] h-[45vw] max-w-[160px] max-h-[160px] md:max-w-none md:max-h-none md:w-64 md:h-64 border-[3px] md:border-[4px] border-black overflow-hidden shadow-[6px_6px_0_rgba(0,0,0,1)] hover:shadow-[8px_8px_0_rgba(255,0,0,1)] transition-all duration-300 cursor-pointer bg-black rounded-sm transform rotate-[-3deg] hover:rotate-0 hover:-translate-y-2 mb-6 md:mb-8 pointer-events-auto">
                            <canvas
                                ref={canvasRef}
                        ...
                        <a href="https://mail.google.com/mail/?view=cm&fs=1&to=vikasmahadev05@gmail.com" target="_blank" rel="noopener noreferrer" className="inline-block bg-white text-black font-['Anton'] text-base md:text-xl px-5 py-2 border-[3px] border-black hover:bg-[#facc15] hover:text-black shadow-[4px_4px_0_rgba(0,0,0,1)] md:shadow-[6px_6px_0_rgba(0,0,0,1)] transition-all duration-300 transform hover:-translate-y-1 hover:rotate-2 cursor-pointer z-30 pointer-events-auto">
                            REACH OUT
                        </a>

                        {/* Static Icons (Always visible below the CTA on all devices) */}
                        <div className="mt-6 md:mt-8 flex flex-wrap justify-center items-center gap-2 md:gap-4 z-30 pointer-events-none">
                            <MobileIconBox type="whatsapp" hoverColor="hover:text-[#25D366] active:text-[#25D366]" />
                            <MobileIconBox type="instagram" hoverColor="hover:text-[#E1306C] active:text-[#E1306C]" />
                            <MobileIconBox type="linkedin" hoverColor="hover:text-[#0A66C2] active:text-[#0A66C2]" />
                            <MobileIconBox type="github" hoverColor="hover:text-gray-800 active:text-gray-800" />
                            <MobileIconBox type="facebook" hoverColor="hover:text-[#1877F2] active:text-[#1877F2]" />
                        </div>
                    </div>
            ...
            <div className="relative w-full h-[10vh] md:h-[20vh] bg-transparent flex items-end justify-center pb-4 md:pb-8 z-10 pointer-events-auto">
